...
public class AdminShopBuy extends Menu {
    ...
    private ItemStack createChangeButton(int amount, Material materials, String displayName) {
        return new ItemBuilder(this, materials, itemMeta -> itemMeta.setDisplayName(displayName))
                .setOnClick(event -> {
                    int newNumber = Math.min(MAX_ITEMS, Math.max(1, number.get() + amount));

                    if (newNumber != number.get()) {
                        number.set(newNumber);
                        updateDisplay(event.getInventory());

                        if (newNumber == MAX_ITEMS) {
