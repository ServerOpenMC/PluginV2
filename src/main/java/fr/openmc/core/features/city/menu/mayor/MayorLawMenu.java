    ...
    public @NotNull Map<Integer, ItemStack> getContent() {
        ...
        try {
            ...
            Supplier<ItemStack> pvpItemSupplier = () -> {
                ...
                if (!DynamicCooldownManager.isReady(mayor.getId().toString(), "mayor:law-pvp")) {
                    loreLawPVP.addAll(
                            List.of(
                                    Component.text(""),
                                    Component.text("§cCooldown §7: " + DateUtils.convertMillisToTime(DynamicCooldownManager.getRemaining(mayor.getId().toString(), "mayor:law-pvp")))
                            )
                    );
                } else {
                ...
                return new ItemBuilder(this, Material.IRON_SWORD, itemMeta -> {
                    ...
                    if (DynamicCooldownManager.isReady(mayor.getId().toString(), "mayor:law-pvp")) {
                        DynamicCooldownManager.use(mayor.getId().toString(), "mayor:law-pvp", COOLDOWN_TIME_PVP);
            ...
            Supplier<ItemStack> warpItemSupplier = () -> {
                ...
                if (!DynamicCooldownManager.isReady(mayor.getId().toString(), "mayor:law-move-warp")) {
                    loreLawWarp.addAll(
                            List.of(
                                    Component.text(""),
                                    Component.text("§cCooldown §7: " + DateUtils.convertMillisToTime(DynamicCooldownManager.getRemaining(mayor.getId().toString(), "mayor:law-move-warp")))
                            )
                    );
                } else {
                    loreLawWarp.addAll(
            ...
            Supplier<ItemStack> announceItemSupplier = () -> {
                ...
                if (!DynamicCooldownManager.isReady(mayor.getId().toString(), "mayor:law-announce")) {
                    loreLawAnnounce.addAll(
                            List.of(
                                    Component.text(""),
                                    Component.text("§cCooldown §7: " + DateUtils.convertMillisToTime(DynamicCooldownManager.getRemaining(mayor.getId().toString(), "mayor:law-announce")))
                            )
                    );
                } else {
                    loreLawAnnounce.addAll(
                ...
                return new ItemBuilder(this, Material.BELL, itemMeta -> {
                    ...
                    if (DynamicCooldownManager.isReady(mayor.getId().toString(), "mayor:law-announce")) {
                        ...
                        DynamicCooldownManager.use(mayor.getId().toString(), "mayor:law-announce", COOLDOWN_TIME_ANNOUNCE);
                    }
            ...
            if (PerkManager.getPerkEvent(mayor) != null) {
                Supplier<ItemStack> perkEventItemSupplier = () -> {
                    ...
                    if (!DynamicCooldownManager.isReady(mayor.getId().toString(), "mayor:law-perk-event")) {
                        lorePerkEvent.addAll(
                                List.of(
                                        Component.text(""),
                                        Component.text("§cCooldown §7: " + DateUtils.convertMillisToTime(DynamicCooldownManager.getRemaining(mayor.getId().toString(), "mayor:law-perk-event")))
                                )
                        );
                    } else {
                        lorePerkEvent.addAll(
                    ...
                    return new ItemBuilder(this, iaPerkEvent, itemMeta -> {
                        ...
                        if (PerkManager.hasPerk(city.getMayor(), Perks.IMPOT.getId())) {
                            for (UUID uuid : city.getMembers()) {
                                if (uuid == city.getMayor().getId()) continue;
                            ...
                            DynamicCooldownManager.use(mayor.getId().toString(), "mayor:law-perk-event", PerkManager.getPerkEvent(mayor).getCooldown());
                            return;
