...
public class CityFeaturesRegistry extends SubRegistry<String, Feature> {
    ...
    public final MayorManager MAYOR = register(new MayorManager(cityManager, fancyNpcsHook));
    public final ProtectionsManager PROTECTIONS = register(new ProtectionsManager(cityManager));
    public final WarManager WAR = register(new WarManager());
    public final CityBankManager CITY_BANK = register(new CityBankManager(cityManager, MAYOR));
    public final CityStatisticsManager STATS = register(new CityStatisticsManager());
    public final NotationManager NOTATION = register(new NotationManager(cityManager));
    public final CityRankManager RANKS = register(new CityRankManager());
    public final CityMilestoneManager CITY_MILESTONE = register(new CityMilestoneManager(cityManager));
    public final CityChatManager CITY_CHAT = register(new CityChatManager());
    public final MascotsManager MASCOTS = register(new MascotsManager());
    public final CityClaimViewManager CLAIM_VIEW = register(new CityClaimViewManager());
